...
- 各ソースのL/Rピークをバーで表示し、その下にOBSの現在のフェーダー位置と`VOL … dB`を表示します。dBFS数値は初期状態では非表示です。アクション設定の「dBFSの数値を表示する」をオンにすると表示します。通常ボタン・ダイヤルごとに設定でき、数値は表示中の2チャンネルの大きい方です。PKの数値とCLIP表示はそのまま残ります。
- ソース一覧は音声ソースだけを表示します。数が多い場合は一覧上部の検索欄で名前を絞り込めます。
...
- バーは即時に立ち上がり、下降時はOBSのMedium（Type I PPM）と同じ20 dB／1.7秒で減衰します。受信間隔に依存しない時間ベースの処理です。フェーダー位置はOBSの設定値を取得して表示し、数値・ピーク検出には受信した実測値を使います。
- バーは−60～0 dBFS。−20 dBFSから黄、−9 dBFSから赤。0 dBFS以上は`CLIP`を1.5秒保持します。
- 白線と`PK`は1.5秒保持するピークです。
...
- 通常ボタン押下、ダイヤル押下、タッチで選択ソースのミュートを切り替えます。失敗時は`ミュート切替エラー`を2秒表示します。
- ダイヤルを右へ回すと選択ソースの音量が上がり、左へ回すと下がります。1目盛り1 dB、範囲は−100～0 dBです。ミュート状態は変更しません。
