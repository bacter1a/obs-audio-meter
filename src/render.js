...
const faderFraction = (db) => Math.max(0, Math.min(1, (db + 100) / 126));
function fader(x, y, width, db, compact = false) {
  ...
  result += text(x + width, labelY, "+26", compact ? 8 : 9, "#62778b", "end");
  return result;
}
...
export function renderMeter({ name = "OBS Audio Meter", levels = [-Infinity, -Infinity], barLevels = levels, peaks = [-Infinity, -Infinity], channels = 2, status = "", clip = false, showDbfs = false, volumeDb }, dial = false) {
  ...
  if (status) {
    ...
    if (Number.isFinite(volumeDb)) {
      content += fader(dial ? 25 : 21, dial ? 63 : 95, dial ? 150 : 112, volumeDb, dial);
      content += text(dial ? 25 : 21, dial ? 98 : 137, `VOL ${formatDb(volumeDb)} dB`, 11, "#9bacbd");
    } else {
    ...
    if (dial) {
      ...
      if (Number.isFinite(volumeDb)) content += fader(25, 63, 150, volumeDb, true);
      content += text(25, 98, Number.isFinite(volumeDb) ? `VOL ${formatDb(volumeDb)} dB` : "VOL --", 11, "#9bacbd");
      content += text(189, 98, clip ? "CLIP" : `PK ${held}`, 11, clip ? "#ff6276" : "#9bacbd", "end");
    } else {
      if (showDbfs) content += text(72, 49, `${db} dBFS`, 19, "#edf4ff", "middle");
      if (Number.isFinite(volumeDb)) content += fader(21, 113, 112, volumeDb);
      content += text(21, 137, Number.isFinite(volumeDb) ? `VOL ${formatDb(volumeDb)} dB` : "VOL --", 11, "#9bacbd");
      content += text(133, 137, clip ? "CLIP" : `PK ${held}`, 11, clip ? "#ff6276" : "#9bacbd", "end");
    }
  }
